  ...
  return (
    <>
      ...
      <div className="absolute top-full right-0 mt-2 z-50" role="dialog" aria-label="Account menu">
        ...
        <div className="bg-[#000000] rounded-sm shadow-xl p-1 md:p-2 flex gap-2">
          {/* Left Content */}
          <div className="w-[180px] sm:w-[160px] md:w-full p-2 md:p-3 bg-[#1c1f2d] rounded-sm">
            ...
            <div className="mb-2">
              <div className="text-[9px] md:text-[10px] font-semibold text-white">{maskValue(email)}</div>
              <div className="text-[8px] md:text-[9px] text-slate-500">ID: {maskValue(String(id))}</div>
            </div>

            <div className="flex items-center gap-2 mb-2">
              <div className="text-[8px] md:text-[9px] text-slate-400">Currency:</div>
              <div className="text-[8px] md:text-[9px] font-semibold bg-slate-700 px-2 py-1 rounded">{currency}</div>
              <button className="text-[8px] md:text-[9px] text-sky-400 px-2 py-1 rounded hover:bg-slate-800"
            ...
            <div className="space-y-2">
              <label className="flex items-start gap-3 bg-[#0b0d10] p-2 rounded border border-transparent hover:border-slate-700 cursor-pointer">
                ...
                <div className="flex-1">
                  <div className="text-[9px] md:text-[10px] font-semibold text-white">Live Account</div>
                  <div className="text-[8px] md:text-[9px] text-slate-500">{hideBalance ? '******' : `₹${fmt(liveBalance)}`}</div>
                  <div className="text-[8px] md:text-[9px] text-slate-500 mt-1">The daily limit is not set</div>
                  <div className="text-[9px] md:text-[10px] text-sky-400 mt-1 cursor-pointer hover:underline" onClick={() => setShowLimitModal(true)}>SET LIMIT</div>
                </div>
              </label>
              ...
              <label className={`flex items-start gap-3 bg-[#0b0d10] p-2 rounded border-2 ${!isLive ? 'border-indigo-800' : 'border-transparent'} cursor-pointer`}>
                ...
                />
                <div className="flex-1">
                  <div className="text-[9px] md:text-[10px] font-semibold text-white">Demo Account</div>
                  <div className="text-[8px] md:text-[9px] text-white font-bold mt-1">${fmt(balance)}</div>
                  <button onClick={handleRefreshDemo} className="text-[8px] md:text-[9px] text-slate-400 mt-1 flex items-center gap-2 hover:text-white ">
          ...
          {/* Right Menu */}
          <div className="w-[80px] sm:w-[60px] md:w-full p-3 md:p-5 bg-[#000000]">

            <ul className="space-y-1">
              {['Deposit', 'Withdrawal', 'Transactions', 'Trades', 'Account'].map((item) => (
                <li key={item}>
                  <button className="w-full text-left px-3 py-2 rounded hover:bg-slate-900 transition-colors text-white text-[9px] md:text-[10px]">
                    {item}
                  </button>
                </li>
              ))}
              <li className="pt-2 border-t border-gray-800 space-y-1">
                {isLive ? (
                 ...
                 text-red-400 text-[9px] md:text-[10px]"
                  >
                    Logout
                  </button>
                ) : (
                  ...
                  <>
                   ...
                   text-primary text-[9px] md:text-[10px]"
                    >
                      Sign Up
                    </button>
                  </>
